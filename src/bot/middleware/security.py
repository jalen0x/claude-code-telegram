...
# Compiled once at module level — used on every message.
...
_PATH_TRAVERSAL_PATTERNS: List[Pattern[str]] = [
    re.compile(p)
    for p in [
        r"\.\./.*",
        r"~\/.*",
...
_SUSPICIOUS_URL_PATTERNS: List[Pattern[str]] = [
    re.compile(p, re.IGNORECASE)
    for p in [
        r"https?://[^/]*\.ru/",
        r"https?://[^/]*\.tk/",
...
_RECON_PATTERNS: List[Pattern[str]] = [
    re.compile(p, re.IGNORECASE)
    for p in [
        r"ls\s+/",
        r"find\s+/",
        ...
        if pat.search(text):
            if audit_logger:
                await audit_logger.log_security_violation(
                    ...
                    details=f"Dangerous pattern detected: {pat.pattern}",
                    severity="high",
        ...
        if pat.search(text):
            if audit_logger:
                await audit_logger.log_security_violation(
                    ...
                    details=f"Path traversal pattern detected: {pat.pattern}",
                    severity="high",
                    attempted_action="message_send",
                )
        ...
        if pat.search(text):
            if audit_logger:
                await audit_logger.log_security_violation(
                    ...
                    details=f"Suspicious URL pattern detected: {pat.pattern}",
                    severity="medium",
                    attempted_action="message_send",
                )

            logger.warning(
                "Suspicious URL detected", user_id=user_id, pattern=pat.pattern
            )
...
async def threat_detection_middleware(
    ...
    # Use module-level compiled patterns
    recon_attempts = sum(1 for pat in _RECON_PATTERNS if pat.search(text))
